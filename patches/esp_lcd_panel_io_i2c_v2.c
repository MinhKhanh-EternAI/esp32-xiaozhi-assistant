...
static esp_err_t panel_io_i2c_rx_buffer(esp_lcd_panel_io_t *io, int lcd_cmd, void *buffer, size_t buffer_size)
{
    ...
    if (send_param) {
        ...
        ESP_GOTO_ON_ERROR(i2c_master_transmit_receive(i2c_panel_io->i2c_handle, write_buffer, write_size, buffer, buffer_size, 100), err, TAG, "i2c transaction failed");
    } else {
        ESP_GOTO_ON_ERROR(i2c_master_receive(i2c_panel_io->i2c_handle, buffer, buffer_size, 100), err, TAG, "i2c transaction failed");
    }
...
static esp_err_t panel_io_i2c_tx_buffer(esp_lcd_panel_io_t *io, int lcd_cmd, const void *buffer, size_t buffer_size, bool is_param)
{
    ...
    if (!is_param && buffer_size > 32) {
        // Pixel data (color) transfer - split into 32-byte chunks to stay within ESP32 classic 13.1ms hardware I2C timeout limit
        size_t chunk_size = 32;
        size_t sent_bytes = 0;
        const uint8_t *data_ptr = (const uint8_t *)buffer;

        while (sent_bytes < buffer_size) {
            size_t to_send = buffer_size - sent_bytes;
            if (to_send > chunk_size) {
                to_send = chunk_size;
            }

            size_t total_size = control_phase_size + to_send;
            uint8_t *temp_buffer = malloc(total_size);
            if (!temp_buffer) {
                tx_err = ESP_ERR_NO_MEM;
                break;
            }
            if (control_phase_size > 0) {
                temp_buffer[0] = control_phase_byte;
            }
            memcpy(temp_buffer + control_phase_size, data_ptr + sent_bytes, to_send);

            tx_err = i2c_master_transmit(i2c_panel_io->i2c_handle, temp_buffer, total_size, 100);
            free(temp_buffer);
        ...
        // Command, parameter or small data transfer - original path
        ...
        size_t total_size = control_phase_size + cmd_buffer_size + lcd_buffer_size;
        uint8_t *temp_buffer = malloc(total_size);
        if (!temp_buffer) {
        ...
        size_t offset = 0;
        if (control_phase_size > 0) {
            temp_buffer[offset++] = control_phase_byte;
        }
        if (cmd_buffer_size > 0) {
            memcpy(temp_buffer + offset, cmd_buffer, cmd_buffer_size);
            offset += cmd_buffer_size;
        }
        if (lcd_buffer_size > 0) {
            memcpy(temp_buffer + offset, lcd_buffer, lcd_buffer_size);
            offset += lcd_buffer_size;
        }
    ...
    if (!is_param) {
        // trans done callback - always call this even on error to prevent LVGL deadlock in wait_for_flushing
        if (i2c_panel_io->on_color_trans_done) {
            i2c_panel_io->on_color_trans_done(&(i2c_panel_io->base), NULL, i2c_panel_io->user_ctx);
        }
    }

    ESP_RETURN_ON_ERROR(tx_err, TAG, "i2c transaction failed");
